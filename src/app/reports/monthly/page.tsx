...
export default function MonthlyReport() {
  ...
  const [chartData, setChartData] = useState<ReportItem[]>([]);
  ...
  useEffect(() => {
    const loadMonthlyReport = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch(`${BASE_URL}/api/reports/monthly?month=${month}&year=${year}`);
        if (!res.ok) throw new Error("Unable to fetch monthly report data");
  ...
  const totalIssued = totals.issued;

  const chartRows = summaryItems.length > 0 ? summaryItems : chartData;
    ...
    <DashboardLayout>
      <div className="p-6 bg-zinc-950 text-white min-h-screen space-y-6">
        ...
        <div className="bg-zinc-900 border border-white/10 rounded-xl p-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between mb-1">
            <div>
              ...
              <p className="text-xs text-gray-500">
                Applications, fees, and cost performance for the selected month.
              </p>
            </div>
          ...
          {/* CUSTOM LEGEND — replaces default Recharts legend */}
        ...
        <div className="text-sm">
          <p>
            Total fees for month of <b>{month}, {year}</b>
          </p>
