...
export interface FeeCalculatedModalProps {
  open: boolean;
  onClose: () => void;
  to: string;
  cc?: string;
...
const FeeCalculatedModal: React.FC<FeeCalculatedModalProps> = ({
  open,
  onClose,
  to,
  cc = "",
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "auto", background: "rgba(0,0,0,0.4)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: 4, width: 750, maxWidth: "98vw", padding: 0, border: "1px solid #f2f2f2", position: "relative", maxHeight: "94vh", overflowY: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: "14px 0 0 0" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", marginBottom: 0 }}>
              ...
              {message && message.includes("<table") ? (
                <div style={{ flex: 1, padding: "8px 8px", border: "1px solid #e5e5e5", borderRadius: 3, background: "#fff", color: "#222", fontWeight: 400, fontSize: 15, fontFamily: "inherit", boxSizing: "border-box", marginTop: 0, width: "100%", outline: "none", boxShadow: "none", overflow: "auto", maxHeight: "60vh" }} // eslint-disable-next-line react/no-danger
                  dangerouslySetInnerHTML={{ __html: message }} />
              ) : (
                <textarea style={{ flex: 1, padding: "8px 8px", border: "1px solid #e5e5e5", borderRadius: 3, background: "#fff", color: "#222", fontWeight: 400, fontSize: 15, fontFamily: "inherit", resize: "none", whiteSpace: "pre-wrap", wordBreak: "break-word", overflow: "hidden", boxSizing: "border-box", marginTop: 0, display: "flex", minHeight: 220, width: "100%", outline: "none", boxShadow: "none" }} readOnly value={message} rows={Math.max(3, message.split('\n').length + 1)} onFocus={(e: React.FocusEvent<HTMLTextAreaElement>) => { e.currentTarget.style.borderColor = '#e5e5e5'; e.currentTarget.style.outline = 'none'; e.currentTarget.style.boxShadow = 'none'; }} onBlur={(e: React.FocusEvent<HTMLTextAreaElement>) => { e.currentTarget.style.borderColor = '#e5e5e5'; e.currentTarget.style.boxShadow = 'none'; }} />
              )}
            </div>
          </div>
        </div>
...
export default FeeCalculatedModal;
