...
export interface CslConfirmationModalProps {
  open: boolean;
  onClose: () => void;
  to: string;
  cc?: string;
...
const CslConfirmationModal: React.FC<CslConfirmationModalProps> = ({
  open,
  onClose,
  to,
  cc = "",
          ...
          <div style={{ padding: "14px 0 0 0" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", marginBottom: 0 }}>
              ...
              {message && /<[^>]+>/.test(message) ? (
                ...
                <textarea style={{ flex: 1, padding: "8px 8px", border: "1px solid #e5e5e5", borderRadius: 3, background: "#fff", color: "#222", fontWeight: 400, fontSize: 15, fontFamily: "inherit", resize: "none", whiteSpace: "pre-wrap", wordBreak: "break-word", overflow: "hidden", boxSizing: "border-box", marginTop: 0, display: "flex", minHeight: 420, width: "100%", outline: "none", boxShadow: "none" }} readOnly value={message} rows={Math.max(3, message.split('\n').length + 1)} onFocus={(e: React.FocusEvent<HTMLTextAreaElement>) => { e.currentTarget.style.borderColor = '#e5e5e5'; e.currentTarget.style.outline = 'none'; e.currentTarget.style.boxShadow = 'none'; }} onBlur={(e: React.FocusEvent<HTMLTextAreaElement>) => { e.currentTarget.style.borderColor = '#e5e5e5'; e.currentTarget.style.boxShadow = 'none'; }} />
              )}
            </div>
          </div>
        </div>
...
export default CslConfirmationModal;
